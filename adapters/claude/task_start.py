#!/usr/bin/env python3
"""Claude Code task-start adapter: read guidance, then capture baseline."""
...
def main() -> int:
    ...
    try:
        instruction_snapshot = prepare_verification_instructions(event, "claude-code")
    except Exception:
        output = (
            {"decision": "block", "reason": PREFLIGHT_FAILURE}
            if event_name == "UserPromptSubmit"
            else {"systemMessage": PREFLIGHT_FAILURE}
        )
