#!/usr/bin/env python3
"""Gemini CLI task-start adapter: read guidance, then capture baseline."""
...
def main() -> int:
    ...
    try:
        instruction_snapshot = prepare_verification_instructions(event, "gemini-cli")
    except Exception:
        output = (
            {"decision": "deny", "reason": PREFLIGHT_FAILURE, "systemMessage": PREFLIGHT_FAILURE}
            if event_name == "BeforeAgent"
            else {"decision": "allow", "systemMessage": PREFLIGHT_FAILURE}
        )
    ...
    messages: list[str] = []
    try:
        capture_task_baseline(event, "gemini-cli")
    except Exception as error:
