...
During `init` or missing-credential recovery in `doctor`, follow **Missing project credential** in [SETUP.md](SETUP.md) before planning. Inspect only root-level filenames and Git metadata for an existing dotenv-style project store; never read its contents. If one safe untracked and ignored regular file exists, authenticate the private-chat key with `scripts/store_project_credential.py --validate-only`, ask the user to place the returned project-derived variable in that file, and stop until a restarted client exposes it through the project's existing loader. Never write or read back the file, guess among several stores, fall back to current-user storage, or invent a loader. If no project store exists, use the documented Windows current-user recovery flow. In either branch, never echo the key or place it in a command line, shell history, client configuration, installer runtime, plan, receipt, log, evidence, or report; failed validation performs no write, and no legacy alias or unrelated project key may be stored.
