...
- For onboarding, first identify the project from the name of its exported `ACCEPTORA_AGENT_TOKEN_PROJ_<ULID>` variable and derive `proj_<ULID>` without printing the credential value. If no matching variable exists, follow **Missing project credential** below instead of ending the turn. If more than one exists and the current worktree is not already bound by a validated receipt, stop and ask the user to identify only the correct variable name; do not guess or request another key.
...
- Confirm the selected project-derived credential variable exists in the coding-agent process environment without printing its value. If it is absent, follow **Missing project credential** below. Do not create a plan first.
...
### Missing project credential

Before asking for a key, inspect only root-level filenames and Git metadata in the canonical target worktree. Never read, source, execute, diff, or print the contents of a possible credential file. Recognize an existing dotenv-style store such as `.env`, `.env.*`, `.dev.vars*`, `.envrc`, or `*.env`; exclude example, sample, template, dist, and defaults files. A usable store must be an ordinary non-link file inside the worktree root that Git confirms is both untracked and ignored. A tracked or unignored dotenv-style file, symlink, junction, reparse point, special file, case-collision, or escaping path is unsafe: stop and ask the user to resolve it. If several usable stores exist, stop and ask which existing file is the project's active environment store; never guess.

If exactly one usable project environment store exists, installation must stop before planning or mutation. Tell the user only its relative path, explain that the key will first be authenticated without storage, and ask the user to paste the Acceptora project key in the private chat. Do not fall back to current-user storage. If no dotenv-style file exists, tell the user that the key will be stored under its project-derived variable name in the Windows current-user environment and that other processes running as the same OS user can read it, then ask the user to paste the key in the private chat. Do not finish with an export command instead of asking.

When the user supplies the key:

1. Treat it as secret input and never repeat it. Run `& "<absolute-python>" -B -I "<source-directory>/scripts/store_project_credential.py"` from the fresh canonical checkout and send the supplied value only to its hidden standard-input prompt. Never add the key to the command. If a usable project environment store was selected, append `--validate-only`. If a validated receipt already selects a project, also append only the non-secret `--expect-project "<receipt-project-id>"` assertion.
2. The helper must validate the exact key shape, derive its project ID and variable name, then authenticate `GET https://www.acceptora.com/api/v1/integrations/project`. It verifies the authenticated project ID and all seven required workflow scopes before any environment write. A malformed, rejected, mismatched, under-scoped, unreachable, or invalid response must leave the environment unchanged.
3. For the project-store branch, `--validate-only` performs no write. After it succeeds, confirm only the returned variable name and project ID, ask the user to add that exact variable under the project's existing syntax to the selected file, and stop. Do not write, read back, diff, or otherwise inspect the file or key. The user must restart the client through the project's already-established environment loader. On the next run, continue only when the exact derived variable is present in the new client process; the file's existence alone is not proof. If no existing loader exposes it, remain stopped rather than duplicating the key into current-user storage or inventing a loader.
4. For the no-project-store branch, the helper stores the validated value on Windows only under the derived project-specific name in `HKEY_CURRENT_USER\Environment`; supplying it after the scoped request authorizes only this credential write. Do not create `ACCEPTORA_AGENT_TOKEN`, store another project's key, or place the value in command-line arguments, shell history, the target worktree, client configuration, installer runtime, plans, receipts, logs, evidence, URLs, or reports. Confirm only the returned variable name and project ID. Restart the selected client so it inherits the new current-user environment, then reread the selected client setup file and this file from a fresh canonical checkout, confirm only that the derived variable is present, and continue the same setup flow.

The packaged helper can validate without persistence on every supported Python platform; its persistent current-user recovery mode supports Windows only. On another operating system with no usable project environment store, report that blocker and do not invent a command or expose the key in process arguments. This recovery does not approve an install or rollback plan, client trust, hooks, MCP permissions, or any product write.
...
The installer never reads or stores the token, grants client trust, or approves tools. The separate credential helper validates the key against Acceptora before either the user places it in an existing ignored project environment store or the helper performs a user-authorized Windows current-user environment write; the installer continues to receive only the derived variable name. A project environment file is never read by the installer and works only when the project's existing loader exports its value into the client process. The installer changes files only after an exact installation-plan digest is supplied. No agent operation can make a human verification decision.
...
Export the agent credential as `ACCEPTORA_AGENT_TOKEN_` plus the uppercase public project ID. For project `proj_01ARZ3NDEKTSV4RRFFQ69G5FAV`, export `ACCEPTORA_AGENT_TOKEN_PROJ_01ARZ3NDEKTSV4RRFFQ69G5FAV`. Outside the explicit **Missing project credential** exchange, never request or place its value in a prompt. Except for the user's validated project-store action described there, never write it to a file, URL, plan, receipt, client configuration, shell history, log, evidence, or report. Repositories and clients connected to the same Acceptora project reuse that stable variable; different projects use different variables.
