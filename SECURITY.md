...
- bearer tokens are loaded from the fixed environment-variable name and are not written to plans, receipts, repository configuration, logs, or output;
...
The operating system, current account, trusted administrators, repository code, agent client, effective hooks, and MCP configuration remain part of the security boundary. Do not install or run the integration on an untrusted fork or pull request.
